...
void prettyPrintT(NodeT *p, int iIndent,TextList textList)
{
    ...
    printf("Type %s ID %s Text %s \n",textList->cType,textList->szId,textList->szText);
    prettyPrint(p->pLeft,iIndent + 1);
 }   
...
NodeT *insertNode(NodeT *p, Element elem, char szParentID, char cYN )
{
    NodeT *t;
    ...
    if(pNew->pRight == NULL && cYN == Y)
        t->pRight = pNew;
    else if (pNew->pLeft == NULL && cYN == Y)
        t->pLeft = pNew;
    else
    {
        printf("Warning that answer is already defined.\n");
        return NULL;
    }    
    return pNew;
}
